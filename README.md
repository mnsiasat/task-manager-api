...
```bash
task-manager-api$ sam local invoke CreateTaskFunction --event events/event.json
```
...
```bash
task-manager-api$ sam logs -n CreateTaskFunction --stack-name task-manager-api --tail
```
